...
RUN zypper addrepo http://download.opensuse.org/repositories/home:illuusio/openSUSE_Tumbleweed/home:illuusio.repo && \
  ...
  find /usr/lib64/python3.6/ \
    \( -name test -o -name tests -o -name 'test_*' -o \
       -name idlelib -o -name fixes -o -name _import_failed -o -name wsgiref -o -name ensurepip -o -name turtle.py -o -name pydoc_data -o -name __pycache__ -o -name sqlite3 -o -name dbm -o -name xmlrpc -o -name email \
    \) -prune -exec rm -rf '{}' '+' && \
  # Clear zypper cache
  zypper clean -a
